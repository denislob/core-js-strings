...
function removeFirstOccurrences(str, value) {
  const segment = str.indexOf(value);
  return str.replace(str.slice(segment, segment + value.length), '');
}
...
function invertCase(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    } else if (str[i].charAt(0) < 97 || str[i].charAt(0) > 122) {
      answer += str[i];
    }
  }
  return answer;
...
function extractNameFromTemplate(value) {
  const answer = value.split(' ').slice(-2).join(' ').replace('!', '');
  return answer;
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    const el = str[i].charCodeAt(0);
    if (el >= 65 && el <= 90) {
      const indexUpp = upp.indexOf(str[i]);
      answer += upp[(indexUpp + 13) % 26];
    } else if (el >= 97 && el <= 122) {
      const indexLow = low.indexOf(str[i]);
      answer += low[(indexLow + 13) % 26];
    } else {
      answer += str[i];
    }
  }
